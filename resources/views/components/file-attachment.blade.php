...
@php
    ...
    $extraDocMimes = [
        '.odt', '.doc', '.docx', '.odp', '.ppt', '.pptx', '.xps', '.txt', '.rtx',
    ];
    ...
    $pdfOrPhoto = implode(',', $pdfMimes) . ',';
    $pdfOrPhoto .= implode(',', $photoMimes);

    $allDocs = implode(',', $pdfMimes) . ',';
    $allDocs .= implode(',', $extraDocMimes) . ',';
    $allDocs .= implode(',', $excelOnly);

    $everything = implode(',', $photoMimes) . ',' . $allDocs;

    $accept = [
        'pdf' => implode(',', $pdfMimes),
        'photo' => implode(',', $photoMimes),
        'excel' => implode(',', $excelMimes),
        'pdfOrPhoto' => $pdfOrPhoto,
        'allDocs' => $allDocs,
        '*' => $everything,
    ][$ext];
    ...
    $extTypes = [
        ...
        'photo' => 'PNG/JPG',
        'excel' => 'XLS / XLSX / CSV',
        'pdfOrPhoto' => 'PDF / PNG / JPG',
        'allDocs' => 'XLS / DOC / XPS / PDF / PPT / TXT / RTX',
        '*' => 'PNG / JPG / XLS / DOC / XPS / PDF / PPT / TXT / RTX',
    ][$ext];
    ...
    handleFiles() {
        if (this.$refs.input) {
            ...
            for(let i = 0; i < files.length; i++) {
                @this.upload('{{ $attributes->wire('model')->value }}', this.$refs.input.files[i], () => {
                }, () => {
                }, (event) => {
        ...
        >
            ...
            <div wire:loading.flex wire:target="{{ $attributes->wire('model')->value }}" wire:loading.class="w-full">
                @if ($mode === 'profile' && $file)
                    <div class="select-none text-sm text-indigo-500 flex items-center justify-center text-center p-4 flex-1">
                ...
                @if ($mode === 'attachment')
                    <div class="text-center flex-1 p-4">
                        <div class="mb-2">{{ trans('components.file_uploader.uploading') }}</div>
            ...
            <div class="flex items-center justify-center flex-1 px-4 py-2" wire:loading.class="hidden" wire:target="{{ $attributes->wire('model')->value }}">
                @if($slot->isEmpty())
                    ...
                    @if ($mode === 'attachment')
                        <span class="ml-2 text-gray-600">{{ is_array($file) ? trans('components.file_uploader.title_multiple') : trans('components.file_uploader.title_single') }} | <span class="text-sm">{{ $extTypes }}</span> | <span class="text-sm">{{ trans('components.file_uploader.max_file_size', ['size' => $maxSize]) }}</span> </span>
                    @endif
                @else
                    {{ $slot }}
                @endif
    ...
    @if ($mode === 'attachment')
        ...
        <div wire:loading.delay wire:loading.flex wire:target="removeUpload" wire:loading.class="w-full">
            <div class="text-sm text-red-500 bg-red-100 flex-1 p-1 text-center rounded">
                {{ trans('components.file_uploader.removing') }}
            </div>
        </div>
                ...
                @foreach($file as $key => $f)
                    <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                        <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg" >
                            @if(collect(['jpg', 'png', 'jpeg'])->contains($f->getClientOriginalExtension()))
